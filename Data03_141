...
library(topicmodels)
#设置工作目录（根据需要修改路径）
setwd("C:\\Myproject_LDA\\database6\\data03_141") 
#加载文档到语料库
#获取目录中的.txt文件列表
...
# 删除通用和自定义的停用词
stopword <- c(stopwords('english'),"plant","women","cancer","ti","ab","can","one","and","like","just","gonna","know","really","right","going","get","well","lot","actually","new",
...
ldaOut.terms <- as.matrix(terms(m,700))
write.csv(ldaOut.terms,file=paste("LDAGibbs",k=4,"TopicsToTerms.csv"))
